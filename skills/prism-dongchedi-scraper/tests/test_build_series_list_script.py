...
class BuildSeriesListScriptTest(unittest.TestCase):
    ...
    def test_build_series_list_preserves_seed_metadata_from_targets(self) -> None:
        skill_dir = Path(__file__).resolve().parent.parent
        script_path = skill_dir / "scripts" / "build_series_list.py"
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            payload = json.loads((tmp_path / "series-list.json").read_text("utf-8"))
            self.assertEqual(payload[0]["brand"], "小米汽车")
            self.assertEqual(payload[0]["energy_type"], "纯电动")
            self.assertFalse(payload[0]["is_history"])
