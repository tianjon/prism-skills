...
def obsidian_list_config_notes(vault: str = "") -> list[str]:
    try:
        result = obsidian_cmd("files", "folder=汽车/参数配置", "ext=md", vault=vault, timeout=30)
...
def main() -> None:
    ...
    existing_notes = set(obsidian_list_config_notes(vault=args.vault))
    new_note_paths = {
        config_note_path(CarConfig(**{k: v for k, v in item.items() if k in CarConfig.model_fields}))
        for item in configs
    }
    ...
    output = {
        ...
        "stats": {
            ...
            "discontinued": len([c for c in changes if c.get("change_type") == "discontinued"]),
        },
    }
